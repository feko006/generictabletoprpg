        ...
        if (encounterState.isSecondFloatingButtonToolbarVisible) {
            HorizontalFloatingToolbar(expanded = true) {
                ...
                if (encounterState.isLegendaryActionButtonVisible) {
                    val maxLegendaryActions
                            by viewModel.maxAvailableLegendaryActionCount.collectAsState(1)
                    if (maxLegendaryActions == 1) {
                        IconButton(onClick = { viewModel.progressInitiativeWithLegendaryAction(1) }) {
                            Icon(boltIcon, "")
                        }
                    } else if (maxLegendaryActions > 1) {
                        ...
                        GttrpgContextMenu(
                            ...
                            for (i in 1..maxLegendaryActions) {
                                DropdownMenuItem(
                                    text = { Text("${i}x") },
                                    onClick = {
                                        viewModel.progressInitiativeWithLegendaryAction(i)
                                        expanded = false
                                    }
                                )
                            }
